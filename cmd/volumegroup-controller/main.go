...
var (
	...
	retryIntervalStart = flag.Duration("retry-interval-start", time.Second, "Initial retry interval of failed volume group creation or deletion. It doubles with each failure, up to retry-interval-max. Default is 1 second.")
	retryIntervalMax   = flag.Duration("retry-interval-max", 5*time.Minute, "Maximum retry interval of failed volume group creation or deletion. Default is 5 minutes.")
)
...
func ensureCustomResourceDefinitionsExist(client *clientset.Clientset) error {
	condition := func() (bool, error) {
		...
		_, err = client.VolumegroupV1alpha1().VolumeGroups("").List(context.TODO(), metav1.ListOptions{})
		if err != nil {
			klog.Errorf("Failed to list v1alpha1 volumegroup with error=%+v", err)
			return false, nil
		}
		_, err = client.VolumegroupV1alpha1().VolumeGroupClasses().List(context.TODO(), metav1.ListOptions{})
		if err != nil {
			klog.Errorf("Failed to list v1alpha1 volumegroupclasses with error=%+v", err)
			return false, nil
		}
		_, err = client.VolumegroupV1alpha1().VolumeGroupContents().List(context.TODO(), metav1.ListOptions{})
		if err != nil {
			klog.Errorf("Failed to list v1alpha1 volumegroupcontents with error=%+v", err)
			return false, nil
		}
		return true, nil
	}
...
func main() {
	...
	if err := ensureCustomResourceDefinitionsExist(volumeGroupClient); err != nil {
